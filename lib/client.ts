...
export default class Client {
  ...
  setupListeners() {
    ipcMain.on("client-node", (event, cmd, data) => {
      ...
      switch (cmd) {
        ...
        case "NOUPDATE":
          if (data === true)
            settings.set_skipCoreUpdate(this.clientConfig.download.sha256); // skip this update
  ...
  IPC_sendCallClientResponse(callId: string, method: string, result: any) {
    if (this.win)
      this.win.webContents.send("client-node", "CALLCLIENT", {
        callId,
        method,
        result,
      });
  ...
  async startClient(restart = false, update = false, commands = []) {
    try {
      ...
      if (!(await helpers.pathExists(this.clientLocalLocation))) {
        ...
        const filehash: string = (await helpers.getFileHash(
          this.clientLocalLocation
        )) as string;
        if (filehash !== this.clientConfig.download.sha256) {
          ...
          if (
            update ||
            settings.getSettings().skipCoreUpdate !==
              this.clientConfig.download.sha256.toUpperCase()
          ) {
            if (update) {
              if (!(await this.downloadClient())) return;
            } else {
  ...
  async getClientBinaries(restart) {
    ...
    if (!restart) {
      try {
        const res: any = await helpers.getRequest(
          "https://raw.githubusercontent.com/thelindaprojectinc/altitude/master/clientBinaries.json"
        );
        let remoteClientBinaries = JSON.parse(res.body);
        if (
          compareVersions(
            remoteClientBinaries[this.clientName].version,
            clientBinaries[this.clientName].version
          ) > 0
        ) {
    ...
    if (
      !clientBinaries[this.clientName][platform] ||
      !clientBinaries[this.clientName][platform][arch]
    ) {
    ...
    this.clientLocalLocation = path.join(
      this.clientsLocation,
      this.clientConfig.bin
    );
  ...
  async downloadClient() {
    try {
      ...
      if (fileHash != this.clientConfig.download.sha256) {
        ...
        await helpers.renameFile(
          this.clientDownloadLocation,
          this.clientLocalLocation
        );
        if (os.platform() !== "win32")
          await helpers.setFileExecutable(this.clientLocalLocation);
        await this.getClientVersion();
      }
      return true;
    } catch (ex) {
  ...
  async waitForClientReady(): Promise<void> {
    ...
    if (
      ...
      this.status === ClientStatus.CLOSEDUNEXPECTED
    ) {
      this.rpcMessage = "";
    ...
    if (!res.success) {
      try {
        if (res.body.error.code === -28)
          this.rpcMessage = res.body.error.message;
      } catch (ex) {
      ...
      if (res.code !== 401) {
        log.info(
          "Client",
          "RPC not ready. retrying in 1000ms",
          this.rpcMessage
        );
        await sleep(1000);
  ...
  async getClientConfig() {
    ...
    try {
      if (await helpers.pathExists(this.clientConfigLocation)) {
        let data = (await helpers.readFile(
          this.clientConfigLocation
        )) as string;
    ...
    try {
      ...
      Object.keys(this.clientConfigFile).forEach((key) => {
        if (
          key !== "nodes" &&
          key !== "rpcport" &&
          this.clientConfigFile[key].toString() !== ""
        )
  ...
  async callClient(method, params = []): Promise<{}> {
    if (method === "getmnsaddress") {
      return new Promise((resolve, reject) => {
        let sender: string = params[0];
        let mnsName: string = params[1];
        this.resolveMNSName(sender, mnsName).then((res) => {
          const result = {
            success: false,
            code: null,
            body: null,
      ...
      return new Promise((resolve, reject) => {
        ...
        request(options, (error, response, body) => {
          ...
          if (error || (body && body.error)) {
            ...
            if (
              this.status === ClientStatus.RUNNINGEXTERNAL &&
              error &&
  ...
  async resolveMNSName(sender: string, mnsName: string) {
  ...
  setClientStatus(status: ClientStatus) {
    ...
    this.status = status;
    if (this.win)
      this.win.webContents.send("client-node", "STATUS", this.status);
  ...
  sendRPCStatus() {
    if (this.win)
      this.win.webContents.send("client-node", "RPC", {
        ready: this.rpcRunning,
  ...
  sendClientVersion() {
    if (this.win)
      this.win.webContents.send("client-node", "VERSION", this.clientVersion);
  }
  ...
  async sendPublicIP() {
    if (this.win)
      this.win.webContents.send("client-node", "IP", await publicIp.v4());
  }
  ...
  async checkClientUpdate() {
    try {
      ...
      if (this.win)
        this.win.webContents.send("client-node", "CHECKUPDATE", hasUpdate);
    } catch (ex) {}
    return;
  }
  ...
  async getClientVersion() {
    if (await helpers.pathExists(this.clientLocalLocation)) {
      this.clientVersion = "";
      this.clientLocalFilehash = (await helpers.getFileHash(
        this.clientLocalLocation
      )) as string;
      if (this.clientVersionHistory) {
        Object.keys(this.clientVersionHistory).forEach((key) => {
          if (
            this.clientVersionHistory[key].indexOf(this.clientLocalFilehash) >
            -1
          )
  ...
  async reportIssue(data) {
    let logLink = "";
    try {
      let mainLogLocation = path.join(
        app.getPath("userData"),
        "logs",
        "main.log"
      );
      let rendererLogLocation = path.join(
        app.getPath("userData"),
        "logs",
        "renderer.log"
      );
    ...
    const baseUrl =
      "https://github.com/TheLindaProjectInc/Altitude/issues/new?body=";
    let body =
      "Description of issue: Please describe the issue with as much information as possible\n\n" +
      "Machine: " +
      os.arch() +
      " " +
      os.platform() +
